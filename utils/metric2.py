...
def mean_dice2(results, gt_seg_maps, num_classes):
    num_imgs = len(results)
    assert len(gt_seg_maps) == num_imgs
    total_dice_mat = []
    dice_metric = {}
    ...
    print("0:",total_dice_mat[:, 0].mean(),"1:",total_dice_mat[:, 1].mean())
    # dice_metric['dice_avg'] = total_dice_mat.mean()
    dice_metric['dice_avg'] = total_dice_mat[:,0].mean()
    print(dice_metric['dice_avg'])
    return dice_metric['dice_avg']
def mean_dice3(results, gt_seg_maps, num_classes):
    num_imgs = len(results)
    assert len(gt_seg_maps) == num_imgs
    total_dice_mat = []
    dice_metric = {}
...
def mean_assd2(results, gt_seg_maps, num_classes):
    num_imgs = len(results)
    assert len(gt_seg_maps) == num_imgs
    total_asd_mat = []
    asd_metric = {}
    ...
    total_asd_mat = np.array(total_asd_mat)
    print("0:",total_asd_mat[:, 0].mean(),"1:",total_asd_mat[:, 1].mean())
    asd_metric['asd_avg'] = (total_asd_mat[:,0].mean())
    print(asd_metric['asd_avg'])

def mean_assd3(results, gt_seg_maps, num_classes):
    num_imgs = len(results)
    assert len(gt_seg_maps) == num_imgs
    total_asd_mat = []
    asd_metric = {}
