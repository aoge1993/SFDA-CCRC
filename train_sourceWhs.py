...
parser.add_argument(
    # '--dataset', type=str, default='Domain_BMC', help='test folder id contain images ROIs to test'
    '--dataset', type=str, default='mr', help='test folder id contain images ROIs to test'
)
parser.add_argument(
    '--model', type=str, default='Deeplab', help='Deeplab'
)
parser.add_argument(
    '--batch-size', type=int, default=24, help='batch size for training the model'   #8->24
)
parser.add_argument(
...
import yaml
from train_process import TrainerWhs
...
def main():
    ...
    if args.no_augmentation:
        domain = Whs_dataloader.WhsSegmentation(base_dir=args.data_dir, dataset=args.dataset,
                                                      split='train', geshi='npy', transform=composed_transforms_ts)
    else:
        domain = Whs_dataloader.WhsSegmentation(base_dir=args.data_dir, dataset=args.dataset,
                                                      split='train', geshi='npy', transform=composed_transforms_tr)
    domain_loader = DataLoader(domain, batch_size=args.batch_size, shuffle=True, num_workers=2, pin_memory=True)

    domain_val = Whs_dataloader.WhsSegmentation(base_dir=args.data_dir, dataset=args.dataset,
                                                      split='test', geshi='nii', transform=composed_transforms_ts)

    domain_loader_val = DataLoader(domain_val, batch_size=args.batch_size, shuffle=False, num_workers=2,
                                   pin_memory=True)

    # 2. model  xinzang-->5
    model = DeepLab(num_classes=5, backbone='mobilenet', output_stride=args.out_stride,
